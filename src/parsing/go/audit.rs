...
impl GoParserAudit {
    ...
    pub fn audit_code(code: &str) -> Result<Self, AuditError> {
        ...
        let mut go_parser =
            GoParser::new().map_err(|e| AuditError::ParserCreation(e.to_string()))?;
        let file_id = FileId(1);
        let mut symbol_counter = crate::types::SymbolCounter::new();
        let symbols = go_parser.parse(code, file_id, &mut symbol_counter);
    ...
    pub fn generate_report(&self) -> String {
        ...
        if !missing.is_empty() {
            ...
            for node in &missing {
                report.push_str(&format!(
                    "- `{node}`: Add example to comprehensive.go or verify node name\n"
                ));
            }
            report.push('\n');
        }
