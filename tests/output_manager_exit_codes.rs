...
fn verify_exit_codes_are_unix_compliant() {
    ...
    let codes = vec![
        ...
        (ExitCode::BlockingError, 2, "Blocking error (Claude Code compatible)"),
    ...
    for (code, expected_value, description) in codes {
        ...
        println!("  {:30} = {:3} ({})", format!("{:?}", code), value, description);
        
        assert_eq!(value, expected_value, "Exit code value mismatch for {:?}", code);
        assert!(value <= 125, "Exit code {} is in reserved range (126-255)", value);
    }
...
fn test_shell_reserved_codes_avoided() {
    ...
    assert!(our_max_code < 126, "We must stay below shell reserved range");
    
    println!("✓ Our max exit code is {}, well below 126", our_max_code);
