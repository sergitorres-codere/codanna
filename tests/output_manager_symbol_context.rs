...
use codanna::io::{ExitCode, OutputFormat};
...
fn create_test_context(id: u32, name: &str, kind: SymbolKind, file: &str, line: u32) -> SymbolContext {
...
fn test_symbol_contexts_with_multiple_items() {
    ...
    let mut manager = OutputManager::new_with_writers(
        OutputFormat::Json,
        Box::new(stdout.clone()) as Box<dyn Write>,
        Box::new(stderr),
    );
    ...
    stdout = manager.stdout.into_inner();
...
fn test_symbol_contexts_text_format() {
    ...
    let contexts = vec![
        create_test_context(1, "process_file", SymbolKind::Function, "src/processor.rs", 100),
        create_test_context(2, "validate_input", SymbolKind::Function, "src/validator.rs", 50),
    ];
...
fn test_symbol_contexts_single_item() {
    ...
    let contexts = vec![
        create_test_context(1, "singleton", SymbolKind::Class, "src/pattern.rs", 42),
    ];
...
fn test_symbol_contexts_preserves_all_fields() {
    ...
    symbol.doc_comment = Some("This function does something important".to_string());
    symbol.signature = Some("fn documented_function(x: i32) -> String".to_string());
