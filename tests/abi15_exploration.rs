...
mod abi15_tests {
    ...
    fn explore_rust_abi15_features() {
        ...
        for node_kind in &["function_item", "impl_item", "struct_item", "trait_item", "mod_item"] {
            let id = language.id_for_node_kind(node_kind, true);
            if id != 0 {
                println!("    {} -> ID: {}", node_kind, id);
            }
        }
        
        // Check field names (useful for extracting specific parts)
        println!("\n  Available Fields: {}", language.field_count());
        for i in 0..5.min(language.field_count() as usize) {
    ...
    fn explore_php_abi15_features() {
        ...
        println!("\n  Key Node Types:");
        for node_kind in &["class_declaration", "function_definition", "method_declaration", "interface_declaration"] {
            let id = language.id_for_node_kind(node_kind, true);
            if id != 0 {
                println!("    {} -> ID: {}", node_kind, id);
            }
    ...
    fn explore_language_behavior_candidates() {
        ...
        let common_concepts = vec![
            ("Function", vec!["function_item", "function_definition", "function_definition"]),
            ("Class", vec!["struct_item", "class_definition", "class_declaration"]),
            ("Method", vec!["function_item", "function_definition", "method_declaration"]),
        ];
